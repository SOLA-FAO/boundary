Boundary repository for the Solutions for Open Land Administration
(SOLA) open source project. This repository contains the Web Service 
Boundary projects including the web services, transfer objects and 
web service clients.

SOLA uses multiple Git repositories for managing its code base. These
The code repository is the super/parent repository. Refer to the 
README in that repository for details on all SOLA Git repositories. 